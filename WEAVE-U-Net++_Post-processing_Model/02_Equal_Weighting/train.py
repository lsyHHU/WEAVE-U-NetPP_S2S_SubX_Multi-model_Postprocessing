...
sys.path.append('/home/liusy/MachineLearning_CSGD/S2S_SubX_Multimodel_250525/03_Unet++_newTest_251209/UNetPP-A-FW-new')
...
file_path = '/data/selfdata/datalsy/S2S_SubX_Multimodel_250525/00_Results_newTest_251209/UNetPP-A-FW-new/'
...
for ib in range(1):   # 重复实验，最后算10次评价指标的平均值
    ...
    for iCV in range(1,13):  
        ...
        while not training_successful:
            ...
            model = UnetPlusPlus(in_ch_list, deep_supervision=False).to(device)
            ...
            with open(log_file_path, 'w', buffering=1) as log_file:
                ...
                # predict
                for fcst_images, geo_images, labels in test_dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        for iday in range(nday_test):
                            ...
                            predict_par_1d = model(fcst_images[iday, ...].unsqueeze(0), 
                                                    geo_images[iday, ...].unsqueeze(0))  # (1, 3, H, W)
