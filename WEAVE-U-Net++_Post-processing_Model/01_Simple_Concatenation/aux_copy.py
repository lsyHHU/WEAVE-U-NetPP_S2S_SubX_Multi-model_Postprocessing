...
sys.path.append('/home/liusy/MachineLearning_CSGD/SubX_Multimodel_250525')
...
class CustomDataset(Dataset):
    ...
    def __init__(self, fcst_images, labels):
    ...
    def __getitem__(self, idx):
        return self.fcst_images[idx], self.labels[idx]


def compute_channel_min_max(dataset):
    fcst_images = []
    for fcst_image, _ in dataset:
        fcst_images.append(fcst_image)
    
    # 将所有图像拼接在一起，计算每个通道的最大最小值
    all_fcst_images = torch.stack(fcst_images)
    fcst_min = all_fcst_images.min(0)[0].min(1)[0].min(1)[0]  # 每个通道的最小值
    fcst_max = all_fcst_images.max(0)[0].min(1)[0].min(1)[0]  # 每个通道的最大值
...
def collate_fn(batch, fcst_min, fcst_max):
    # batch 是一个由 (fcst_images, labels) 组成的列表
    fcst_images, labels = zip(*batch)
...
def model_train(epoch, num_epochs, model, optimizer, loss_fn, train_dataloader, mask, device):
    train_loss_batch = []
    """模型训练阶段"""
    for batch_idx, (fcst_images, labels) in enumerate(
