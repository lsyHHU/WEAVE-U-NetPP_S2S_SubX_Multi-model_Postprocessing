...
from WeightPredictor import CrossAttentionWeightPredictorVarIn
...
class UnetPlusPlus(nn.Module):
    """
    模型输入: 
    1. 273通道 (S2S+SubX multimodel total precip ensemble + atmospheric varibales emsmean) 空间范围48*80
    2. 18个模型的预见期 lead

    不同模型的输入为加权和（根据不同模型的预见期设置可学习权重）

    模型输出:
    当前空间范围的CSGD分布三个参数的空间分布 (3通道)
    """
    
    def __init__(self, in_ch_list, temperature, deep_supervision=False):
        super(UnetPlusPlus, self).__init__()
        self.deep_supervision = deep_supervision

        # 权重预测网络
        self.weight_predictor = CrossAttentionWeightPredictorVarIn(
            in_ch_list=[6+2,8+2,4+2,9+2,9+2,8+2,7+2,13+2,13+2,13+2,13+2,13+2,13+2,12+2,11+2,9+2,12+2,11+2], 
            n_models=18,
            embed_dim=512,
    ...
    def forward(self, fcst, geo, lead, pr_ensmean, pr_ensstd):
        ...
        for i in range(18):
            ...
            atmos.append(torch.cat([pr_ensmean[:, i, :, :].unsqueeze(1),
                                    pr_ensstd[:, i, :, :].unsqueeze(1),
                                    fcst[:, pre_end_idx[i]:atmos_end_idx[i], :, :], 
                                    geo_embed], dim=1))

        # 权重预测网络
        weight, _ = self.weight_predictor(atmos, lead)

        # 模型各自下采样
        x1 = [self.encoder_eachModel_1[i](pre[i]) for i in range(18)]  # n * (B, 128, 48, 80)
        x1 = torch.stack(x1, dim=1)
        x1_w = torch.einsum('bnchw,bnhw->bchw', x1, weight)  # (B, 128, 48, 80)

        x2 = [self.encoder_eachModel_2[i](self.avgpool2(pre[i])) for i in range(18)]  # n * (B, 256, 24, 40)
        x2 = torch.stack(x2, dim=1)
        x2_w = torch.einsum('bnchw,bnhw->bchw', x2, self.avgpool2(weight))  # (B, 256, 24, 40)

        x3 = [self.encoder_eachModel_3[i](self.avgpool3(pre[i])) for i in range(18)]  # n * (B, 512, 12, 20)
        x3 = torch.stack(x3, dim=1)
        x3_w = torch.einsum('bnchw,bnhw->bchw', x3, self.avgpool3(weight))  # (B, 512, 12, 20)

        x4 = [self.encoder_eachModel_4[i](self.avgpool4(pre[i])) for i in range(18)]  # n * (B, 1024, 6, 10)
        x4 = torch.stack(x4, dim=1)
        x4_w = torch.einsum('bnchw,bnhw->bchw', x4, self.avgpool4(weight))  # (B, 1024, 6, 10)

        # 公共下采样
        x_0_0 = x1_w      # (B, 128, 48, 80)
        x_1_0 = self.stage1(self.pool(x_0_0)) + x2_w  # (B, 256, 24, 40)
        x_2_0 = self.stage2(self.pool(x_1_0)) + x3_w  # (B, 512, 12, 20)
        x_3_0 = self.stage3(self.pool(x_2_0)) + x4_w  # (B, 1024, 6, 10)
        ...
        if self.deep_supervision:
            ...
            return weight, torch.cat([shift.unsqueeze(1), mu.unsqueeze(1), sigma.unsqueeze(1)], dim=1)
...
# lead = torch.randint(1, 5, (2, 18), dtype=torch.float32)
# pr_ensmean = torch.randn(2, 18, 48, 80)
# pr_ensstd = torch.randn(2, 18, 48, 80)

# in_ch_list = [10, 3, 4, 4, 11, 1, 4, 33, 4, 10, 11, 4, 11, 10, 4, 8, 3, 7]

# model = UnetPlusPlus(in_ch_list, temperature=0.1, deep_supervision=False)
# w, o = model(fcst, geo, lead, pr_ensmean, pr_ensstd)
