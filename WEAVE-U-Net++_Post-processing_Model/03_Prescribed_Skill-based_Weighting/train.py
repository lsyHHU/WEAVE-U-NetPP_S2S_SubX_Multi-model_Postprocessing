...
sys.path.append('/home/liusy/MachineLearning_CSGD/S2S_SubX_Multimodel_250525/03_Unet++_newTest_251209/UNetPP-A-SkillBasedWeight-NoWeightLearning')
...
file_path = '/data/selfdata/datalsy/S2S_SubX_Multimodel_250525/00_Results_newTest_251209/UNetPP-A-SkillBasedWeight-NoWeightLearning/'
...
# 读取原始降水预报crps数据
crps_arr = np.load(f'/data/selfdata/datalsy/S2S_SubX_Multimodel_250525/00_Results/crps_by_model_{tscale}days.npy')  # (nday, nmodel, nlat, nlon)
...
for ib in range(1):   # 重复实验，最后算10次评价指标的平均值
    ...
    for iCV in range(9,10):  
        ...
        # 根据训练期的obs对crps进行normalization
        ncrps_arr_train = crps_arr_train / np.nanmean(obs_arr_train, axis=0)
        # 根据训练期的平均crps计算模型权重（固定）
        err_ref = np.nanmean(ncrps_arr_train, axis=0)   # (M, H, W)
        err_ref_train = np.broadcast_to(err_ref[None,...], (len(train_issuedate_idx),)+err_ref.shape)
        err_ref_test = np.broadcast_to(err_ref[None,...], (len(test_issuedate_idx),)+err_ref.shape)

        train_dataset = CustomDataset(
            torch.tensor(fcst_arr_train, dtype=torch.float32),
            torch.tensor(geo_arr_train, dtype=torch.float32),
            torch.tensor(err_ref_train, dtype=torch.float32),
        ...
        test_dataset = CustomDataset(
            ...
            torch.tensor(err_ref_test, dtype=torch.float32),
        ...
        while not training_successful:
            ...
            # 初始化模型
            T = 0.1
            in_ch_list = [13+3, 8+3, 5+3, 10+3, 17+3, 6+3, 8+3, 43+3, 14+3, 20+3, 21+3, 14+3, 21+3, 19+3, 12+3, 14+3, 12+3, 15+3]
            model = UnetPlusPlus(in_ch_list, T=T, deep_supervision=False).to(device)
            ...
            # 早停、优化器、损失函数
            early_stopping = EarlyStopping(patience=10, delta=-0.1, verbose=False, start_early_stop_epoch=50)
            ...
            with open(log_file_path, 'w', buffering=1) as log_file:
                ...
                # predict
                for fcst_images, geo_images, crps, labels in test_dataloader:
                    ...
                    with torch.no_grad():
                        ...
                        for iday in range(nday_test):
                            ...
                            w, predict_par_1d = model(fcst_images[iday, ...].unsqueeze(0), 
                                                    geo_images[iday, ...].unsqueeze(0),
                                                    crps[iday, ...].unsqueeze(0))  # (1, 3, H, W)
