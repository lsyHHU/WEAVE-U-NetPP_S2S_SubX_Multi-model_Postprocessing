...
class CustomDataset(Dataset):
    ...
    def __init__(self, fcst_images, geo_images, crps, labels):
    ...
    def __getitem__(self, idx):
        return self.fcst_images[idx], self.geo_images[idx], self.crps[idx], self.labels[idx]


def compute_channel_min_max(dataset):
    fcst_images = []
    for fcst_image, _, _, _ in dataset:
...
def collate_fn(batch, fcst_min, fcst_max):
    # batch 是一个由 (fcst_images, geo_images, crps, labels) 组成的列表
    fcst_images, geo_images, crps, labels = zip(*batch)
    ...
    return fcst_images, geo_images, crps, labels
...
def model_train(epoch, num_epochs, model, optimizer, loss_fn, train_dataloader, mask, device):
    train_loss_batch = []
    """模型训练阶段"""
    for batch_idx, (fcst_images, geo_images, crps, labels) in enumerate(
        ...
        if device == 'cpu':
            ...
            fcst_images, geo_images, labels = fcst_images.to(device), geo_images.to(device), labels.to(device)
            crps = crps.to(device)
            mask = mask.to(device)
        ...
        optimizer.zero_grad()
        # 前向传播
        _, predict_par = model(fcst_images, geo_images, crps)
