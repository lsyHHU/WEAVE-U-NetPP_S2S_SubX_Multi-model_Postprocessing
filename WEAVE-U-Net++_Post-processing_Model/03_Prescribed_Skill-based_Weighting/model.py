...
class UnetPlusPlus(nn.Module):
    ...
    不同模型的输入为加权和（根据训练期样本CRPS技能反比得到的权重，没有进行权重预测训练）

    模型输出:
    当前空间范围的CSGD分布三个参数的空间分布 (3通道)
    """
    
    def __init__(self, in_ch_list, T, deep_supervision=False):
    ...
    def forward(self, fcst, geo, err_ref):
        ...
        # fcst分开每个模型
        start_idx = [0, 13, 21, 26, 36, 53, 59, 67, 110, 124, 144, 165, 179, 200, 219, 231, 245, 257]
        end_idx = [13, 21, 26, 36, 53, 59, 67, 110, 124, 144, 165, 179, 200, 219, 231, 245, 257, 272]

        pre = []
        for i in range(18):
            pre.append(fcst[:, start_idx[i]:end_idx[i], :, :])
        # 每个模型输入与geo_embed拼接
        pre = [torch.cat([pre[i], geo_embed], dim=1) for i in range(18)]

        # skill-based weight for each model
        err_nan2num = torch.nan_to_num(err_ref, nan=999)  # (B, 18, 48, 80)
        weight, _ = compute_oracle_weights(err_nan2num, temperatue=self.T)  # (B, 18, 48, 80) 
...
# crps_arr = np.load(f'/data/selfdata/datalsy/S2S_SubX_Multimodel_250525/00_Results/crps_by_model_7days.npy')[:2]
# crps = torch.tensor(crps_arr, dtype=torch.float32)

# in_ch_list = [13+3, 8+3, 5+3, 10+3, 17+3, 6+3, 8+3, 43+3, 14+3, 20+3, 21+3, 14+3, 21+3, 19+3, 12+3, 14+3, 12+3, 15+3]
# model = UnetPlusPlus(in_ch_list, 1.0, False)
# w, o = model(fcst, geo, crps)
