...
file_path = '/data/selfdata/datalsy/S2S_SubX_Multimodel_250525/00_Results_newTest_251209/UNetPP-A-WeightPredictor-T/'
...
for ib in range(1):   # 重复实验，最后算10次评价指标的平均值
    ...
    for iCV in range(1):  
        ...
        while not training_successful:
            ...
            model = UnetPlusPlus(in_ch_list, temperature=0.1, deep_supervision=False).to(device)
            ...
            with open(log_file_path, 'w', buffering=1) as log_file:
                ...
                # predict
                for fcst_images, geo_images, labels, leads, pr_ensmean_images, pr_ensstd_images in test_dataloader:
