...
for t in range(4):
    ...
    for m in range(5):
        ...
        if m != 4:
            if m == 0:  # raw (ECMWF)
                file_name = f'D:/00_Code_lsy/MachineLearning_CSGD/S2S_SubX_Multimodel_250525/{model_name[m]}/{_t}days/raw_crpss_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears', model='ECMWF').squeeze()
                raw_metrix_file.append(metrix_file)
            elif m == 1:  # MME
                file_name = root_dir + f'{model_name[m]}/{_t}days/mme_crpss_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears').squeeze()
                mme_metrix_file.append(metrix_file)
            elif m == 2:  # emos
                file_name = root_dir + f'{model_name[m]}/{_t}days/emos_crpss_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears').squeeze()
                emos_metrix_file.append(metrix_file)
            else:   # Unet-based
                file_name = root_dir + f'{model_name[m]}/Output_0/{_t}days/post_crpss_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears').squeeze()
            ...
            # 在每个子图中再插入一个小子图，绘制当前空间分布crpss的频率直方图
        ...
        ################
            ...
            axes[t][m].set_xlabel('CRPSS (%)', fontsize=24) if t == 3 else None
...
# 添加统计数据
bold_mean_idx = get_bold_value_index(metrix_name='CRPSS', metrix_values_arr=metrix_mean_arr)
...
p_cb.set_label(label='CRPSS (%)', fontdict=font3)
...
# 保存图片
fig.savefig(root_dir + 'draw_251226/supple_pic_260521/s05_crpss_map.jpg')
