...
for t in range(4):
    ...
    postfcst.close()
    postfcst = postfcst.sel(lat=basin_lat, lon=basin_lon, issuetime=slice('2009-1-1', '2009-12-31')).squeeze()
    ...
    emosfcst_regional_mean_arr = emosfcst_regional_mean.values   # (n,)
    # 集合平均
    ...
    mmefcst_regional_mean_arr = mmefcst_regional_mean.values   # (n,)
    # 集合平均
    ...
    rawfcst_regional_mean_arr = rawfcst_regional_mean.values   # (n,)
    # 集合平均
    ...
    for i in range(5):
        ...
        ax[t, i].set_xlim(pd.to_datetime('2009-01-01'), pd.to_datetime('2009-12-31'))
        ax[0, i].set_ylim(-5, 40)
        ax[1, i].set_ylim(-5, 50)
        ax[2, i].set_ylim(-5, 70)
        ax[3, i].set_ylim(-5, 100)
...
for t in range(4):
    for i in range(5):
        ...
        ax[t, i].legend(loc='upper left', fontsize=16) if t == 0 else None
...
ax[0, 0].text(pd.to_datetime('2008-05-20'), 20, '7 days', fontsize=22, fontweight='bold')
ax[1, 0].text(pd.to_datetime('2008-05-20'), 25, '14 days', fontsize=22, fontweight='bold')
ax[2, 0].text(pd.to_datetime('2008-05-20'), 35, '21 days', fontsize=22, fontweight='bold')
ax[3, 0].text(pd.to_datetime('2008-05-20'), 50, '28 days', fontsize=22, fontweight='bold')
...
fig.savefig(savedir + f'82_precip_fcst_time_series_example_{basin}_2009_new.jpg', dpi=600)
