...
def calculate_improvement(metrix_name, raw_metrix_da, post_metrix_da):
    ...
    elif metrix_name in ['RMSE', 'nRMSE']:
        metrix_improve = (post_metrix_da < raw_metrix_da).sum().values / ngrid * 100.
    elif metrix_name == 'RB':
...
def add_hist_inset(parent_ax, data, bins, xlim, ylim, cmap, norm):
    ...
    hist_ax.set_xticks([xlim[0], xlim[1]])
...
gs = gridspec.GridSpec(4, 5, width_ratios=[1, 1, 1, 1, 1.6], wspace=0.0, hspace=0.2)
...
for t in range(4):
    ...
    for m in range(5):
        ...
        if m != 4:
            if m == 0:  # raw (ECMWF)
                file_name = f'D:/00_Code_lsy/MachineLearning_CSGD/S2S_SubX_Multimodel_250525/{model_name[m]}/{_t}days/raw_nrmse_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears', model='ECMWF').squeeze()
                raw_metrix_file.append(metrix_file)
            elif m == 1:  # MME
                file_name = root_dir + f'{model_name[m]}/{_t}days/mme_nrmse_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears').squeeze()
                mme_metrix_file.append(metrix_file)
            elif m == 2:  # emos
                file_name = root_dir + f'{model_name[m]}/{_t}days/emos_nrmse_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears').squeeze()
                emos_metrix_file.append(metrix_file)
            else:   # Unet-based
                file_name = root_dir + f'{model_name[m]}/Output_0/{_t}days/post_nrmse_{_t}daysTP_Year20012013.nc'
                metrix_file = xr.open_dataarray(file_name).sel(year='AllYears').squeeze()
            ...
            HIST_BINS = np.arange(0.2, 1.6, 0.1)
            HIST_XLIM = (0.2, 1.6)
            HIST_YLIM = (0.0, 0.3)
            HIST_CMAP = 'YlOrRd'
            HIST_NORM = Normalize(vmin=0.2, vmax=1.6)
        ...
        ################
            ...
            axes[t][m].plot(unet_metrix_sorted, unet_cdf, color=colors[3], label='WEAVE-U-Net++', marker='P', markevery=100, lw=1.5, ms=6)
            axes[t][m].axvline(0, color='gray', linewidth=1.5)
            axes[t][m].set_xlim(0.2, 2.0)
            ...
            axes[t][m].set_xlabel('nRMSE', fontsize=24) if t == 3 else None
            axes[t][m].tick_params(labelsize=20)
            axes[t][m].legend(loc='lower right', fontsize=16) if t == 0 else None
        ...
        ################
        # 添加列标题和行标题
        axes[t][m].text(25, 30, row_title[t], fontdict=font2) if m == 0 else None
        axes[t][m].set_title(column_title[m], fontdict=font2, pad=30.0) if t == 0 else None


# 添加统计数据
bold_mean_idx = get_bold_value_index(metrix_name='nRMSE', metrix_values_arr=metrix_mean_arr)
...
p_cb.set_label(label='nRMSE', fontdict=font3)
...
# 保存图片
fig.savefig(root_dir + 'draw_251226/supple_pic_260521/s04_nrmse_map.jpg')
