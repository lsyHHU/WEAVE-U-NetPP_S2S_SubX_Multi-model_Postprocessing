...
for t in range(4):
    ...
    for m in range(5):
        ...
        if m == 1:
            file_name = f'D:/00_Code_lsy/MachineLearning_CSGD/S2S_SubX_Multimodel_250525/{model_name[m]}/Output_0/{_t}days/post_crpss_{_t}daysTP_Year20012013.nc'
        else:
            file_name = filedir + f'{model_name[m]}/Output_0/{_t}days/post_crpss_{_t}daysTP_Year20012013.nc'
        ...
        # WEAVE-U-Net++ 模型绘制crpss地图
        if m == 0:
            p_crpss = metrix_file.plot.imshow(ax=axes[t][m], 
        ...
        # 其他模型，绘制与WEAVE-U-Net++之间的CPRSS差值
            ...
            q_crpss = diff.plot.imshow(ax=axes[t][m], 
                        transform=ccrs.PlateCarree(), x='lon', y='lat', 
                        vmin=-10, vmax=10, 
                        extend='both', cmap='RdBu_r', add_colorbar=False,
                        levels=np.linspace(-10, 10, 11))
            # 统计 diff > 0 (即WEAVE-U-Net++模型表现更好) 的网格占比, delta crpss > 0 代表技能变差
...
p_crpss_ax_cb = fig.add_axes([ax_3_0_pos.x0+ax_3_0_pos.width+0.02, ax_3_0_pos.y0+0.02, 0.015, 0.82])
p_crpss_cb = fig.colorbar(
    p_crpss, 
    cax=p_crpss_ax_cb, 
    extend='max', 
    drawedges=True, 
...
p_crpss_cb.set_label(label='CRPSS (%)', fontdict=font3, labelpad=15)
p_crpss_ax_cb.tick_params(which='major', direction='in', length=7)
for ticklabel in p_crpss_ax_cb.yaxis.get_ticklabels():
    ticklabel.set_fontsize(22)

# delta crpss
ax_3_4_pos = axes[3][4].get_position()
q_crpss_ax_cb = fig.add_axes([ax_3_4_pos.x0+ax_3_4_pos.width+0.02, ax_3_4_pos.y0+0.02, 0.015, 0.82])
q_crpss_cb = fig.colorbar(
    q_crpss, 
    cax=q_crpss_ax_cb, 
    extend='both', 
    drawedges=True, 
...
q_crpss_cb.set_label(label=r'$\Delta_{\mathrm{CRPSS}}$' + '(%)', fontdict=font3, labelpad=15)
q_crpss_ax_cb.tick_params(which='major', direction='in', length=7)
for ticklabel in q_crpss_ax_cb.yaxis.get_ticklabels():
    ticklabel.set_fontsize(22)


# 存储
fig.savefig(filedir + f'draw_251226/supple_pic_260521/s09_crpss_ablation_comparison_map.jpg')
