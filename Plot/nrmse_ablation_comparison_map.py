...
for t in range(4):
    ...
    for m in range(5):
        ...
        if m == 1:
            file_name = f'D:/00_Code_lsy/MachineLearning_CSGD/S2S_SubX_Multimodel_250525/{model_name[m]}/Output_0/{_t}days/post_nrmse_{_t}daysTP_Year20012013.nc'
        else:
            file_name = filedir + f'{model_name[m]}/Output_0/{_t}days/post_nrmse_{_t}daysTP_Year20012013.nc'
        ...
        # WEAVE-U-Net++ 模型绘制NRMSE地图
        if m == 0:
            p_nrmse = metrix_file.plot.imshow(ax=axes[t][m], 
                        transform=ccrs.PlateCarree(), x='lon', y='lat', 
                        vmin=0, vmax=1.5, 
                        extend='max', cmap='YlOrRd', add_colorbar=False,
                        levels=np.linspace(0, 1.5, 16))
        # 其他模型，绘制与WEAVE-U-Net++之间的CPRSS差值
        else:
            diff = metrix_file - metrix_file_arr[0]
            q_nrmse = diff.plot.imshow(ax=axes[t][m], 
                        transform=ccrs.PlateCarree(), x='lon', y='lat', 
                        vmin=-0.1, vmax=0.1, 
                        extend='both', cmap='RdBu', add_colorbar=False,
                        levels=np.linspace(-0.1, 0.1, 11))
            # 统计 diff > 0 (即WEAVE-U-Net++模型表现更好) 的网格占比, delta NRMSE > 0 代表技能变差
            per = np.sum(np.where(diff > 0, 1, 0)) / 928 * 100
            axes[t][m].text(120, 56, f'{per:.2f}%', fontdict=font1)
            # 在每个子图中再插入一个小子图，绘制当前空间分布BSS的频率直方图
            HIST_BINS = np.linspace(-0.1, 0.1, 11)
            HIST_XLIM = (-0.1, 0.1)
            HIST_YLIM = (0.0, 0.4)
            HIST_CMAP = 'RdBu'
            HIST_NORM = Normalize(vmin=-0.1, vmax=0.1)
...
p_nrmse_ax_cb = fig.add_axes([ax_3_0_pos.x0+ax_3_0_pos.width+0.02, ax_3_0_pos.y0+0.02, 0.015, 0.82])
p_nrmse_cb = fig.colorbar(
    p_nrmse, 
    cax=p_nrmse_ax_cb, 
    extend='max', 
    drawedges=True, 
...
p_nrmse_cb.set_label(label='nRMSE', fontdict=font3, labelpad=15)
p_nrmse_ax_cb.tick_params(which='major', direction='in', length=7)
for ticklabel in p_nrmse_ax_cb.yaxis.get_ticklabels():
    ticklabel.set_fontsize(22)

# delta NRMSE
ax_3_4_pos = axes[3][4].get_position()
q_nrmse_ax_cb = fig.add_axes([ax_3_4_pos.x0+ax_3_4_pos.width+0.02, ax_3_4_pos.y0+0.02, 0.015, 0.82])
q_nrmse_cb = fig.colorbar(
    q_nrmse, 
    cax=q_nrmse_ax_cb, 
    extend='both', 
    drawedges=True, 
...
q_nrmse_cb.set_label(label=r'$\Delta_{\mathrm{nRMSE}}$', fontdict=font3, labelpad=15)
q_nrmse_ax_cb.tick_params(which='major', direction='in', length=7)
for ticklabel in q_nrmse_ax_cb.yaxis.get_ticklabels():
    ticklabel.set_fontsize(22)


# 存储
fig.savefig(filedir + f'draw_251226/supple_pic_260521/s08_nrmse_ablation_comparison_map.jpg')
