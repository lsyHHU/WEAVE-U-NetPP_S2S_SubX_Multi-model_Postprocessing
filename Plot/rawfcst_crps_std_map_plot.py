...
for t in range(4):
    ...
    std_da = cprs_nc.std(dim='model', skipna=True)
    
    # 季节循环
    for isea in range(2):
        if isea == 0:
            std_da_1sea = std_da.sel(issuetime=std_da.issuetime.dt.month.isin([4, 5, 6, 7, 8, 9]))
        else:
            std_da_1sea = std_da.sel(issuetime=std_da.issuetime.dt.month.isin([10, 11, 12, 1, 2, 3]))
        # 取时间维度平均
        std_da_1sea_mean = std_da_1sea.mean(dim='issuetime', skipna=True)


        # 绘图
        p = std_da_1sea_mean.plot.imshow(
            ax = axes[isea][t],
