...
# 得到最优值的索引
crpss_max_idx = np.argmax(data_arr, axis=1)  # 4 tscales, 10 regions
...
crpss_norm = mpl.colors.Normalize(vmin=-30, vmax=30)
crpss_cmap = mpl.cm.get_cmap('RdBu_r')

# 绘制热力图
for t in range(4):
    q = ax[t].pcolormesh(data_arr[t, :, :], cmap=crpss_cmap, norm=crpss_norm, alpha=0.75, edgecolor='k')
    # 子图设置
    ...
    for i in range(4):   # 4 models
        for j in range(10):   # 10 regions
            val = data_arr[t, i, j] 
            rgba = crpss_cmap(crpss_norm(val)) 

            # 如果是最大值，则用蓝色加粗
            if crpss_max_idx[t, j] == i:
                ax[t].text(
...
# 添加colorbar
q_ax_cb = fig.add_axes([ax[2].get_position().x0, 
...
q_cb.set_label(label='CRPSS (%)', fontsize=26)
...
# 保存图片
fig.savefig(path + 'draw_251226/pic_251226/72-1_rawfcst_postfcst_crpss_skill_heatmap_(easy_version).jpg', dpi=600)
